...
class StockPicking(models.Model):
    ...
    def process_barcode_from_ui(self, barcode_str, visible_op_ids):
        ...
        matching_location_ids = stock_location_obj.search(
            [("barcode", "=", barcode_str)]
        )
    ...
    def action_assign_owner(self):
        for picking in self:
            ...
            self.env["stock.move.line"].write(
                packop_ids, {"owner_id": picking.owner_id.id}
            )
    ...
    def action_done_from_ui(self, picking_id):
        ...
        for operation in picking.move_line_ids:
            operation.with_context(no_recompute=True).write(
                {"product_uom_qty": operation.qty_done}
            )
        picking.do_transfer()
        # return id of next picking to work on
        return self.get_next_picking_for_ui(picking.picking_type_id.id)
...
class StockPickingType(models.Model):
    ...
    def open_barcode_interface(self):
        final_url = (
            "/barcode/web/#action=stock.ui&picking_type_id=" + str(self.ids[0])
            if len(self.ids)
            else "0"
        )
    ...
    def _increment(
        ...
        package_clause = [
            (
                "result_package_id",
                "=",
                self.env.context.get("current_package_id", False),
            )
        ]
        existing_operation_ids = self.search(
            [("picking_id", "=", picking_id)] + domain + package_clause
        )
        todo_operation_ids = []
        if existing_operation_ids:
            if filter_visible:
                todo_operation_ids = [
                    val for val in existing_operation_ids if val.id in visible_op_ids
                ]
            else:
...
class QuantPackage(models.Model):
    ...
    def do_print_package(self):
        return self.env.ref(
            "stock_picking_barcode.action_report_package_stock_picking_barcode"
        ).report_action(self)
